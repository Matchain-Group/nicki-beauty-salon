          ...
          {filteredProducts.map((product) => (
            <div key={product._id} className="bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow">
              <div className="relative">
                ...
                >
                  <svg
                    className={`w-4 h-4 ${wishlist.has(product._id) ? 'fill-red-500 text-red-500' : 'text-gray-400'}`}
                    fill="currentColor"
                    viewBox="0 0 20 20"
                  >
            ...
            <div className="border-t p-6">
              ...
              <div className="grid grid-cols-4 gap-4">
                {filteredProducts.slice(0, 4).map((product) => (
                  <div key={product._id} className="cursor-pointer" onClick={() => setSelectedProduct(product)}>
                    <Image
                      src={getProductImage(product)}
                      alt={product.title}
                      width={150}
